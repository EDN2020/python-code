...
resource "aws_cloudwatch_log_group" "_" {
  name              = "/aws/lambda/${local.name}-lambda-loggroup"
...
resource "aws_lambda_function" "_" {
  ...
  handler          = "cloudwatch_alert.lambda_handler"
  runtime          = "python3.9"
  timeout          = 60
...
resource "aws_cloudwatch_event_rule" "_" {
  ...
  description         = "cloudwatch-alert lambda schedule"
